...
export default function Update() {
  ...
  const page = 1
  const pageSize = 10
  const datalog = useSelector((state) => state.runWorkflow.userData)
  ...
  useEffect(() => {
    dispatch(fetchRunWorkflowUserAsync(userId))
  }, [userId])
  ...
  const columns = [
...

    {
      ...
      render: (_, record) => (
        <Space size="middle">
          <a> {moment(record.run_time).format('LLL')}</a>
        </Space>
      ),
    },
    {
  ...
  const handleTableRowClick = (record) => {
    const newPath = `/u/workflow/${record.project_id}`
      ...
      <div className="p-3 h-screen overflow-hidden">
        <div className="p-3">
          <div className="flex items-center justify-between mb-10">
            <h2 className="text-xl font-semibold mb-2">History Workflows</h2>
          ...
          <div className="h-full pb-20 overflow-auto scrollable-content">
            <Table
              columns={columns}
