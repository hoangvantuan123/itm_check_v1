...
import NewProject from '../components/sildebar-frame/new-project'
import decodeJWT from '../../utils/decode-JWT'
import { fetchSynWorkflowUser } from '../../features/syn/fetch-syn'
import { fetchCodeUserDeploy } from '../../features/app-code/fetch-app-code'
import { setAppCode } from '../../features/app-code/slice/codeSlice'
import { deleteProject } from '../../features/app-code/delete-project'
...
export default function AppCode() {
  ...
  const projectData = useSelector((state) => state.appCode.data)
  ...
  const fetchData = async () => {
    try {
      const response = await dispatch(fetchCodeUserDeploy(userId))
      dispatch(setAppCode(response.payload))
    } catch (error) {
  ...
  const handleDelete = async (record) => {
    ...
    try {
      setIsLoading(false)
      await dispatch(deleteProject(record.id))
      ...
      <div>
        <div className="p-3  h-screen overflow-hidden">
          ...
          <div className="  h-full  pb-20 flex  flex-col flex-1 overflow-auto scrollable-content scroll-container">
            <Title level={5}>Individual App Code</Title>
            <Table
              columns={columns}
