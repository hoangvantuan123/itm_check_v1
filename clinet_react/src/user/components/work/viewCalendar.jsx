...
export default function CalendarView({ sampleData, value, setValue, setDrawerVisible, setCheckInOutHistory }) {
  const { t } = useTranslation()

  const onSelect = (newValue) => {
    const selectedDate = newValue.tz('Asia/Ho_Chi_Minh').format('YYYY-MM-DD')
    setValue(newValue)
    const data = sampleData.find((item) => item.date === selectedDate)?.records || []
