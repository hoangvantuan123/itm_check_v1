...
import EditTrigger from '../workflows/view-edit/trigger'
import ShowMoreTrigger from './show-more-trigger'
...
function TemplateTrigger({ id, data, isConnectable }) {
          ...
          <div className="p-2">
            ...
            <div className=" border w-full rounded-lg bg-slate-50 p-1">
              <div className=" w-full rounded p-1 ">
                <Text> New every day - Schedule:</Text>
                <p className="opacity-75  text-sm">
                  Minute: {nodenew?.details?.schedule?.minute} <br />
                  Hour: {nodenew?.details?.schedule?.hour} <br />
                  Day: {nodenew?.details?.schedule?.day} <br />
                  Week: {nodenew?.details?.schedule?.week} <br />
                  Month: {nodenew?.details?.schedule?.month} <br />
                  Day Of Week: {nodenew?.details?.schedule?.dayOfWeek}
                </p>
              </div>
              <div className="h-[0.5px]  bg-slate-200 w-full mb-1 rounded-lg"></div>
              <EditTrigger data={data} nodeId={id} />
            </div>
          </div>
        </>
      )}
