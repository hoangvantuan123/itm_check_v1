...
import EditFunction from '../workflows/view-edit/function'
import AceEditor from 'react-ace'
import ShowMoreFunction from './show-more-function'
...
function TemplateFunction({ id, data, isConnectable }) {
  ...
  const [code, setCode] = useState(nodenew?.details?.code_function?.code || '')
  const [output, setOutput] = useState('')
  const [error, setError] = useState('')
  const [timeoutId, setTimeoutId] = useState(null)

  useEffect(() => {
    setCode(nodenew?.details?.code_function?.code)
  }, [nodenew])
  ...
  const handleChangeJS = useCallback(
    (evt) => {
      const formattedCode = formatCode(evt)
    ...
    <div className=" h-auto w-96 border rounded-lg p-1  bg-white">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-1 p-1 ">
          {alignValue == 'Javascript' && <JavascriptIcon />}
          {alignValue == 'Python' && <PythonIcon />}
        ...
        <>
          <div className="p-2">
            ...
            <div className=" border w-full rounded-lg bg-slate-50 p-1">
              ...
              <EditFunction data={data} code={code} idNode={id} />
            </div>
          </div>
        </>
      )}
